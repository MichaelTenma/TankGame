...
//	private final int v = speed/time;//暂时速度是一个定值,以后可能会考虑到功率的问题
	...
	public int command = TANKGAME_CODE.tankUpCode;
	...
	public void tankMove(int command) {
		...
		if(this.isLive){
			if(pass){
				...
				ForbiddenObstacle fo = new ForbiddenObstacle(this.tankName, origin, this.tankWidth, this.tankHeight,TANKGAME_CODE.thingType_TANK);
				if(Data_ControlObstacle.forbiddenArea.size()>0){
					Vector<ForbiddenObstacle> tempVector = dco.getForbiddenObstacleByName(this.tankName);
//					System.out.println(tempVector.size());
					for(int i = 0;i < tempVector.size();i++){
						if(tempVector.get(i).getType()==TANKGAME_CODE.thingType_TANK){
							dco.removeForbiddenObstacle(tempVector.get(i));
						}
					}
				}
				dco.addForbiddenObstacle(fo);
				
				for(int k = 0;k < Data_ControlObstacle.forbiddenArea.size();k++){
					ForbiddenObstacle temp = Data_ControlObstacle.forbiddenArea.get(k);
					if(!(temp.getObstacleName().equals(this.getTankName()))){
