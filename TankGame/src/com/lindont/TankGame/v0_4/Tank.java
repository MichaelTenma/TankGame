		...
		if(this.isLive){
			if(pass){
				...
				for(int k = 0;k < Data_ControlObstacle.forbiddenArea.size();k++){
					...
					Thing thingTemp = tempForbiddenObstacle.getThing();
//					if(thingTemp.getType()==TANKGAME_CODE.thingType_TANK){
//						Tank tankTemp = (Tank)thingTemp;
						
						if(!thingTemp.getName().equals(this.getName())){
							int obstacleX = thingTemp.getX(),obstacleY = thingTemp.getY();
							int tempX = obstacleX-this.getWidth();
							int tempY = obstacleY-this.getHeight();
...
//		int tankWidth_Ball=this.getPanelWidth()/80;
		...
		this.setWidth(tankWidth);this.setHeight(tankHeight);
//		g.fill3DRect(0, 0,tankWidth,tankHeight , false);//测试获取到的坦克宽高
		...
		if(runTimes>0){
			...
			if(this.isLive){
				...
				ForbiddenObstacle forbiddenObstacle = new ForbiddenObstacle(this,TANKGAME_CODE.thingType_TANK);
